...
class cXCA(nn.Module):
    ...
    def forward(self, x, y):
        ...
        kv = self.kv(y).reshape(B, N, 2, self.num_heads, C // self.num_heads)
...
class dcXCA(nn.Module):
    ...
    def forward(self, x, y):
        B, N, C = x.shape
        q = self.q(x).reshape(B, N, 1, self.num_heads, C // self.num_heads)
        qkv = self.qkv(y).reshape(B, N, 3, self.num_heads, C // self.num_heads)
        q = q.permute(2, 0, 3, 1, 4)
        qkv = qkv.permute(2, 0, 3, 1, 4)
        q, q_y, k, v = q[0], qkv[0], qkv[1], qkv[2]
        q = q * torch.sigmoid(q_y)
...
class dXCA(nn.Module):
    ...
    def forward(self, x, w):
        ...
        w = w.reshape(B, N, 1, self.num_heads, C // self.num_heads)
        w = w.permute(2, 0, 3, 1, 4)
        w = w[0]
        q = q * w
        k = k * w
...
class CATransformer(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., expan_ratio=6,
        ...
        self.norm_x = LayerNorm(self.dim, eps=1e-6)
        self.norm_y = LayerNorm(self.dim, eps=1e-6)

        self.cxca = cXCA(self.dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, y):
        ...
        if self.pos_embd:
            pos_encoding = self.pos_embd(B, H, W).reshape(B, -1, y.shape[1]).permute(0, 2, 1)
            x = x + pos_encoding
            y = y + pos_encoding

        y = y + self.cxca(self.norm_x(x), self.norm_y(y))
...
class DynamicAttention(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., expan_ratio=6,
        ...
        self.dxca = dXCA(self.dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, w):
        ...
        B, C, H, W = x.shape
        x = x.reshape(B, C, H * W).permute(0, 2, 1)
        w = w.reshape(B, C, H * W).permute(0, 2, 1)

        if self.pos_embd:
            pos_encoding = self.pos_embd(B, H, W).reshape(B, -1, x.shape[1]).permute(0, 2, 1)
            x = x + pos_encoding

        x = x + self.dxca(self.norm_xca(x), w)
...
class DynamicCrossAttention(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., expan_ratio=6,
        ...
        self.dcxca = dcXCA(self.dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x, y):
        ...
        x = x + self.dcxca(self.norm_x(x), self.norm_y(y))
...
class MixFFN(nn.Module):
    def __init__(self, dim, kernel_size=3, padding=1):
        ...
        self.dim = dim
        self.norm = LayerNorm(self.dim, eps=1e-6)
        self.cr0 = nn.Conv2d(self.dim, self.dim, 1)
        self.depthwise_conv = nn.Conv2d(in_channels=self.dim, out_channels=self.dim, kernel_size=kernel_size,
                                        padding=padding, bias=True, groups=self.dim)
        self.relu = nn.ReLU(inplace=True)
        self.cr1 = nn.Conv2d(self.dim, self.dim, 1)
